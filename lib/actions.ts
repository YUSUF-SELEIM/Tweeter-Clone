...
export async function getTweetStats(tweetId: string) {
  try {
    // Fetch comments, likes, and retweets counts in parallel
    const [commentsCount, likesCount, retweetsCount] = await Promise.all([
    ...
    // Return an object with all the counts
    return {
...
export async function getLikeStatusOfComment(
  commentId: string,
  userId: string
) {
  const existingLike = await prisma.commentLike.findFirst({
